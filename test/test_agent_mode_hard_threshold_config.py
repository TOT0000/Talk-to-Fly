...
def test_hardgate_prompt_has_extra_hard_gate_rule():
    assert 'If predicted_collision_probability > 0.5, you MUST output response=full_replan_plan with a new complete MiniSpec plan.' in PLANNER_SOURCE
...
def test_heartbeat_prompt_contains_budget_and_timing_policy():
    assert 'The system calls you once every 7 seconds during execution.' in SOFT_PROMPT
    assert 'The system calls you once every 7 seconds during execution.' in HARD_PROMPT
