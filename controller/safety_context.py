...
class SafetyContext:
    ...
    per_obstacle_collision_probabilities: Optional[List[Dict[str, Any]]] = None
    collision_debug_info: Optional[Dict[str, Any]] = None

    def to_prompt_block(self) -> str:
        per_obstacle_probs = self.per_obstacle_collision_probabilities or []
        per_obstacle_prob_block = (
            "\n".join(
                f"- {row.get('id')}: P_c={float(row.get('collision_probability')):.6f}"
                for row in per_obstacle_probs
            )
            if per_obstacle_probs
            else "- (n/a)"
        )
        return (
