...
RUN_COLUMNS = [
    ...
    "completion_time_including_all_llm_response_sec",
    "completion_time_excluding_all_llm_response_sec",
    "completion_time_excluding_in_mission_llm_wait_sec",
    "total_replan_count",
    "accepted_replan_count",
    "full_replan_response_count",
    "replan_requested_count",
    "replan_interrupt_count",
    "replan_execution_resume_count",
    "replan_discarded_count",
    "replan_discard_reason_counts",
    ...
    "total_all_llm_response_time_sec",
    "initial_planning_latency_sec",
    "total_initial_planning_latency_sec",
    "total_replan_response_time_sec",
    "total_heartbeat_response_time_sec",
    "total_event_predrisk_response_time_sec",
    "total_periodic_infoaware_response_time_sec",
    "total_agent_heartbeat_response_time_sec",
    "total_evaluator_response_time_sec",
    "llm_wait_event_count",
    "gc_llm_wait_pause_count",
    "gc_llm_wait_total_sec",
    "gc_llm_wait_resume_count",
    "gc_llm_wait_replan_preempt_count",
    "event_predrisk_threshold_check_count",
    "event_predrisk_trigger_count",
    "event_predrisk_pending_trigger_count",
    "event_predrisk_trigger_consumed_count",
    "event_predrisk_wait_count",
    "event_predrisk_wait_total_sec",
    "event_predrisk_replan_request_count",
    "event_predrisk_replan_applied_count",
    "event_predrisk_invalid_response_count",
    "event_predrisk_suppressed_count",
    "event_predrisk_suppress_reason_counts",
    "event_predrisk_latest_trigger_probability",
    "event_predrisk_latest_trigger_source",
    "event_predrisk_latest_trigger_target",
    "event_predrisk_latest_plan",
...
class TaskRunLogger:
    ...
    def _build_runtime_trace_row(self, snapshot: Dict, now: float) -> Dict:
        ...
        return {
            ...
            "event_predrisk_checked": bool(snapshot.get("event_predrisk_checked", False)),
            "event_predrisk_triggered": bool(snapshot.get("event_predrisk_triggered", False)),
            "event_predrisk_trigger_source": snapshot.get("event_predrisk_trigger_source"),
            "event_predrisk_suppressed_reason": snapshot.get("event_predrisk_suppressed_reason"),
            "event_predrisk_pending_trigger": bool(snapshot.get("event_predrisk_pending_trigger", False)),
            "event_predrisk_current_threshold": snapshot.get("event_predrisk_current_threshold"),
            "event_predrisk_strictly_greater": bool(snapshot.get("event_predrisk_strictly_greater", False)),
        }
    ...
    def get_pending_run_summary(self) -> Dict:
        ...
        return {
            ...
            "gc_llm_wait_pause_count": int(final_mission_summary.get("gc_llm_wait_pause_count", 0) or 0),
            "gc_llm_wait_total_sec": float(final_mission_summary.get("gc_llm_wait_total_sec", 0.0) or 0.0),
            "gc_llm_wait_resume_count": int(final_mission_summary.get("gc_llm_wait_resume_count", 0) or 0),
            "gc_llm_wait_replan_preempt_count": int(final_mission_summary.get("gc_llm_wait_replan_preempt_count", 0) or 0),
    ...
    def _persist_run(self, active: _RunRecord):
        ...
        row = {
            ...
            "completion_time_including_all_llm_response_sec": final_mission_summary.get("completion_time_including_all_llm_response_sec", completion_time_mission_sec),
            "completion_time_excluding_all_llm_response_sec": final_mission_summary.get("completion_time_excluding_all_llm_response_sec"),
            "completion_time_excluding_in_mission_llm_wait_sec": final_mission_summary.get("completion_time_excluding_in_mission_llm_wait_sec", final_mission_summary.get("completion_time_excluding_llm_wait_sec", completion_time_mission_sec or 0.0)),
            "total_replan_count": int(final_mission_summary.get("replan_count", (final or {}).get("replan_count", 0)) or 0),
            "accepted_replan_count": int(final_mission_summary.get("accepted_replan_count", 0) or 0),
            "full_replan_response_count": int(final_mission_summary.get("full_replan_response_count", 0) or 0),
            "replan_requested_count": int(final_mission_summary.get("replan_requested_count", 0) or 0),
            "replan_interrupt_count": int(final_mission_summary.get("replan_interrupt_count", 0) or 0),
            "replan_execution_resume_count": int(final_mission_summary.get("replan_execution_resume_count", 0) or 0),
            "replan_discarded_count": int(final_mission_summary.get("replan_discarded_count", 0) or 0),
            "replan_discard_reason_counts": self._json_text(final_mission_summary.get("replan_discard_reason_counts", {})),
            ...
            "total_all_llm_response_time_sec": float(final_mission_summary.get("total_all_llm_response_time_sec", 0.0) or 0.0),
            "initial_planning_latency_sec": float(final_mission_summary.get("initial_planning_latency_sec", 0.0) or 0.0),
            "total_initial_planning_latency_sec": float(final_mission_summary.get("total_initial_planning_latency_sec", 0.0) or 0.0),
            "total_replan_response_time_sec": float(final_mission_summary.get("total_replan_response_time_sec", 0.0) or 0.0),
            "total_heartbeat_response_time_sec": float(final_mission_summary.get("total_heartbeat_response_time_sec", 0.0) or 0.0),
            "total_event_predrisk_response_time_sec": float(final_mission_summary.get("total_event_predrisk_response_time_sec", 0.0) or 0.0),
            "total_periodic_infoaware_response_time_sec": float(final_mission_summary.get("total_periodic_infoaware_response_time_sec", 0.0) or 0.0),
            "total_agent_heartbeat_response_time_sec": float(final_mission_summary.get("total_agent_heartbeat_response_time_sec", 0.0) or 0.0),
            "total_evaluator_response_time_sec": float(final_mission_summary.get("total_evaluator_response_time_sec", 0.0) or 0.0),
            "llm_wait_event_count": int(final_mission_summary.get("llm_wait_event_count", 0) or 0),
            "gc_llm_wait_pause_count": int(final_mission_summary.get("gc_llm_wait_pause_count", 0) or 0),
            "gc_llm_wait_total_sec": float(final_mission_summary.get("gc_llm_wait_total_sec", 0.0) or 0.0),
            "gc_llm_wait_resume_count": int(final_mission_summary.get("gc_llm_wait_resume_count", 0) or 0),
        ...
        run_summary = {
            ...
            **llm_latency_summary,
            ...
            "gc_llm_wait_pause_count": row.get("gc_llm_wait_pause_count", 0),
            "gc_llm_wait_total_sec": row.get("gc_llm_wait_total_sec", 0.0),
            "gc_llm_wait_resume_count": row.get("gc_llm_wait_resume_count", 0),
            "gc_llm_wait_replan_preempt_count": row.get("gc_llm_wait_replan_preempt_count", 0),
            "trajectory_sample_count": trajectory_sample_count,
            "trajectory_buffer_source": trajectory_buffer_source,
            "trajectory_sampler_interval_sec": trajectory_sampler_interval_sec,
            "trajectory_sampler_active_during_run": trajectory_sampler_active_during_run,
