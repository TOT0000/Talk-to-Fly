...
class TaskRunLogger:
    ...
    def _build_runtime_trace_row(self, snapshot: Dict, now: float) -> Dict:
        ...
        return {
            ...
            "statement_end_reason": snapshot.get("statement_end_reason"),
            "interrupted_for_replan": bool(snapshot.get("interrupted_for_replan", False)),
            "entered_awaiting_replan_response": bool(snapshot.get("entered_awaiting_replan_response", False)),
            "old_queue_drained": bool(snapshot.get("old_queue_drained", False)),
            "next_statement_executed_after_interrupt": bool(snapshot.get("next_statement_executed_after_interrupt", False)),
            "replan_wait_start_ts": snapshot.get("replan_wait_start_ts"),
            "replan_response_commit_ts": snapshot.get("replan_response_commit_ts"),
            "execution_resumed_from_new_plan": bool(snapshot.get("execution_resumed_from_new_plan", False)),
