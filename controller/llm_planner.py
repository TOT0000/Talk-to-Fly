...
class LLMPlanner():
    def __init__(self, robot_type: RobotType):
        ...
        self.prompt_langgraph_mode_action = (
            ...
            "- If collision risk toward current_subgoal is high, DO NOT output go_checkpoint(current_subgoal) immediately.\n"
            "- In high-risk cases, choose recovery first with movement/turn actions (move_forward/move_backward/move_left/move_right/turn_cw/turn_ccw or equivalent) to detour around workers, increase distance, or change approach geometry.\n"
            "- After each recovery step, re-observe the latest state and latest collision risk before choosing the next action.\n"
            "- Return to approach/go_checkpoint(current_subgoal) only after the updated state shows risk has dropped enough to resume mission progress safely.\n"
